...
pub async fn scrape_debank(address: &str) -> Result<(), Box<dyn Error>> {
    ...
    println!("url:{url}    value:{value:?}", url=url, value=text);
...
pub async fn scrape_doge_explorer(address: &str) -> Result<(), Box<dyn Error>> {
    let driver = initialize_driver().await?;

    let base = Url::parse("https://blockchair.com/dogecoin/address/")?;
    ...
    let elem = driver.find(By::XPath("/html/body/div[2]/div[2]/div/div[2]/div/div/div/div/span[3]")).await?;
    let text = elem.text().await?;
    println!("url:{url}    value:{value:?}", url=url, value=text);
...
async fn initialize_driver() -> Result<WebDriver, WebDriverError> {
    ...
    let driver = WebDriver::new("http://localhost:50547", caps).await?;
