...
void ripemd160_process( RIPEMD160_CTX *ctx, __private const uint8_t data[64] )
{
...
void ripemd160_Update( RIPEMD160_CTX *ctx, __private const uint8_t *input, uint32_t ilen )
{
    ...
    if( left && ilen >= fill )
    {
        ...
        ripemd160_process( ctx, (__private const const uint8_t *)ctx->buffer );
        input += fill;
        ilen  -= fill;
        left = 0;
    }
...
void ripemd160(__private const uint8_t *msg, uint32_t msg_len, uint8_t hash[20])
{
    RIPEMD160_CTX ctx;
    ripemd160_Init( &ctx );
